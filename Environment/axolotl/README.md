...
This repository contains a modified version of `Axolotl 0.8.1` that has been verified to work. Please follow the following instructions for an editable install.

```
# Create and activate a new conda environment
mamba create -y -n Axolotl python=3.12 ipython
mamba activate Axolotl
mamba install -y ccache git libaio
mamba install -y -c https://software.repos.intel.com/python/conda/ -c conda-forge oneccl-devel
```

```
# Install the latest torch with CUDA 12.8
pip3 install torch torchvision
```

```
# Install important dependencies 
pip install uv ninja psutil packaging einops msgpack numpy tqdm hjson pydantic pydantic-core annotated-types nvitop
```

```
# Install FashAttention
...
DS_BUILD_OPS=1 DS_BUILD_SPARSE_ATTN=0 DS_BUILD_STOCHASTIC_TRANSFORMER=0 DS_BUILD_EVOFORMER_ATTN=0 pip install --no-build-isolation deepspeed --config-settings "--global-option=build_ext" --config-settings "--global-option=-j128"
```

```
# Install Axolotl
pip3 install --no-build-isolation -e ".[ring-flash-attn,deepspeed]"
```
